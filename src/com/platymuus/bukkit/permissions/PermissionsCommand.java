...
final class PermissionsCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] split) {
		...
		if (subcommand.equals("reload")) {
			 ...
			 * "Metrics are disabled."); return true; }
			 * 
			 * sender.sendMessage(ChatColor.GOLD +
			 * "Features Used:"); for (Map.Entry<String, String>
			 * entry : plugin.getMetrics().summarize(0).entrySet())
			 * { sender.sendMessage("  " + ChatColor.GREEN +
			 * entry.getKey() + ": " + ChatColor.WHITE +
			 * entry.getValue()); }
			 * sender.sendMessage(ChatColor.GOLD + "Usage:"); for
			 * (Map.Entry<String, String> entry :
			 * plugin.getMetrics().summarize(1).entrySet()) {
			 * sender.sendMessage("  " + ChatColor.GREEN +
			 * entry.getKey() + ": " + ChatColor.WHITE +
			 * entry.getValue()); }
			 */
			...
			if (page == -1) {
				...
				try {
					...
					// right now permissible is always a
					// CommandSender
			...
			// This is essentially player setgroup with an added
			// check
