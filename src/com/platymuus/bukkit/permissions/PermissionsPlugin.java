...
public final class PermissionsPlugin extends JavaPlugin {
	...
	// private final PermissionsMetrics metrics = new
	// PermissionsMetrics(this);
	...
	public void onEnable() {
		...
		// Metrics are fun!
		/*
		 * try { metrics.start(); } catch (IOException ex) {
	...
	public void saveConfig() {
		// If there's no keys (such as in the event of a load failure)
		// don't save
	...
	protected void refreshForGroup(String group) {
		...
		// e.g. if Bob is only a member of "expert" which inherits
		// "user", he
		...
		for (UUID uuid : permissions.keySet()) {
			...
			// if the player isn't in the config, act like they're
			// in default
		...
		if (sec != null) {
			for (String child : sec.getKeys(false)) {
				...
				if (node != null && (name.equals(node.getString("name")) || name.equals("child"))) {
					// either the "name" field matches or
					// the key matches
					return node;
				}
			}
	...
	protected void calculateAttachment(Player player) {
		...
		// Fill the attachment reflectively so we don't recalculate for
		// each permission
	...
	// normally, LinkedHashMap.put (and thus putAll) will not reorder the
	// list
	// if that key is already in the map, which we don't want - later puts
	// should
	...
	private Map<String, Boolean> calculatePlayerPermissions(Player player, String world) {
		...
		// if the player isn't in the config, act like they're in
		// default
		if (node == null) {
		...
		// first, apply the player's groups (getStringList returns an
	...
	public boolean SetGroup(Player player, String setgroups) {
		UUID playerUUID = player.getUniqueId();// resolvePlayer(sender,
							// split[2]);
		if (playerUUID == null)
			return false;
		String[] groups = setgroups.split(",");
		createNode("users/" + playerUUID).set("groups", Arrays.asList(groups));
		refreshForPlayer(playerUUID);
		return true;
	}

	public boolean AddGroup(Player player, String setgroup) {
		UUID playerUUID = player.getUniqueId();
		if (playerUUID == null)
			return false;
		String group = setgroup;
		List<String> list = createNode("users/" + playerUUID).getStringList("groups");
		if (list.contains(group)) {
			Bukkit.broadcast(ChatColor.GREEN + "Player " + ChatColor.WHITE + player.getName() + ChatColor.GREEN + " was already in " + ChatColor.WHITE + group + ChatColor.GREEN + ".", "permissions.player.*");
			return false;
		}
		list.add(group);
		getNode("users/" + playerUUID).set("groups", list);
		refreshForPlayer(playerUUID);
		Bukkit.broadcast(ChatColor.GREEN + "Player " + ChatColor.WHITE + player.getName() + ChatColor.GREEN + " is now in " + ChatColor.WHITE + group + ChatColor.GREEN + ".", "permissions.player.*");
		return true;
	}

	public boolean RemoveGroup(Player player, String removegroup) {
		UUID playerUUID = player.getUniqueId();
		if (playerUUID == null)
			return false;
		// Bukkit.broadcastMessage("Remove!");
		String group = removegroup;
		List<String> list = createNode("users/" + playerUUID).getStringList("groups");
		if (!list.contains(group)) {
			Bukkit.broadcast(ChatColor.GREEN + "Player " + ChatColor.WHITE + player.getName() + ChatColor.GREEN + " was not in " + ChatColor.WHITE + group + ChatColor.GREEN + ".", "permissions.player.*");
			return true;
		}
		list.remove(group);
		getNode("users/" + playerUUID).set("groups", list);
		refreshForPlayer(playerUUID);
		Bukkit.broadcast(ChatColor.GREEN + "Player " + ChatColor.WHITE + player.getName() + ChatColor.GREEN + " is no longer in " + ChatColor.WHITE + group + ChatColor.GREEN + ".", "permissions.player.*");
		return true;
	}
	...
	private Map<String, Boolean> calculateGroupPermissions0(Set<String> recursionBuffer, String group, String world) {
		...
		// first apply any parent groups (see calculatePlayerPermissions
		// for more)
