...
final class PermissionsTabComplete implements TabCompleter {
	...
	public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
		// Remember that we can return null to default to online player
		// name matching

		/*
		 * reload - reload the configuration from disk. check <node>
		 * [player] - check if a player or the sender has a permission
		 * (any plugin). info <node> - prints information on a specific
		 * permission. dump [player] [page] - prints info about a
		 * player's (or the sender's) permissions. setrank <player>
		 * <group> - set a player to be in a group with per-group
		 * permissions. group - list group-related commands. player -
		 * list player-related commands.
		 */
		...
		if (args.length <= 1) {
			...
			String sub = args[0];
			// note that dump is excluded here because there's no
			// real reason to tab-complete page numbers
	...
	private List<String> groupComplete(CommandSender sender, String[] args) {
		...
		String lastArg = args[args.length - 1];
		/*
		 * group list - list all groups. group players <group> - list
		 * players in a group. group setperm <group> <[world:]node>
		 * [true|false] - set a permission on a group. group unsetperm
		 * <group> <[world:]node> - unset a permission on a group.
		 */
		...
		if (sub.equals("players")) {
			...
			if (args.length == 3) {
				...
				// TODO: maybe only show nodes that are already
				// set?
				return worldNodeComplete(lastArg);
			}
		}
	...
	private List<String> playerComplete(CommandSender sender, String[] args) {
		...
		String lastArg = args[args.length - 1];
		/*
		 * player groups <player> - list groups a player is in. player
		 * setgroup <player> <group,...> - set a player to be in only
		 * the given groups. player addgroup <player> <group> - add a
		 * player to a group. player removegroup <player> <group> -
		 * remove a player from a group. player setperm <player>
		 * <[world:]node> [true|false] - set a permission on a player.
		 * player unsetperm <player> <[world:]node> - unset a permission
		 * on a player.
		 */

		// A convenience in case I later want to replace online players
		// with something else
		...
		if (sub.equals("groups")) {
			...
			if (args.length == 3) {
				...
				if (idx == -1) {
					...
					String done = lastArg.substring(0, idx + 1); // includes
											// the
											// comma
